...
async def _run_graph(entry: str):
    ...
    stream = graph.stream({"messages": ["Hello"]}, stream_mode="messages", config=config)
...
async def start():
    ...
    res = await cl.AskActionMessage(
        ...
        actions=[
            cl.Action(label="Make a new plan with the Coach", payload={"value" : "new_plan"}, name="action_button"),
            cl.Action(label="Modify your plan with the Coach", payload={"value" : "discuss"}, name="action_button"),
        ],
    ).send()
...
async def chat_stream(user_msg: cl.Message):
    ...
    for msg_chunk, _meta in stream:
        if isinstance(msg_chunk, AIMessageChunk) and msg_chunk.content and msg_chunk.additional_kwargs.get("visible", True) and _meta["langgraph_node"] != "search":
            await out_msg.stream_token(str(msg_chunk.content))

        if isinstance(msg_chunk, AIMessage) and not isinstance(msg_chunk, AIMessageChunk):
