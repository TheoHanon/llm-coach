...
def init_llms() -> Dict[str, Any]:
    llm_small = init_chat_model(
        model=settings.model_small, model_provider=settings.model_provider
    )
    llm_large = init_chat_model(
        model=settings.model_large, model_provider=settings.model_provider
    )
