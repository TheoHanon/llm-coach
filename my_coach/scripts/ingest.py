...
def enrich_metadata(docs : List[Document]) -> List[Document]:
    ...
    for d in docs:
        ...
        if src:
            ...
            try: d.metadata.setdefault("category", pathlib.Path(src).relative_to(CORPUS_DIR).parts[0])
...
def build_index(chunks: List[Document]):
    emb = MistralAIEmbeddings(model="mistral-embed", api_key=os.environ["MISTRAL_API_KEY"])
